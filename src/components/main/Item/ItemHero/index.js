...
import { getUSDTBalance,getETHBalance, getMATICBalance, getUSDTBalanceForMatic } from '../../../../contract/web3'
...
export const ItemHero = (props) => {
  ...
  const creatorOptions = [
    ...
    { value: 'matic', content: <Option><span className='name'>MATIC</span></Option> }, 
    { value: 'tst', content: <Option><span className='name'>TST</span></Option> }, 
    ]
  ...
  const upgradeHandler = async()=>{
    ...
    else if(type == 'wallet')
    {
      if(currency == 'algo'){
        ...
        if(price > balance[4]){
          toast("Your MATIC balance is lower(current - "+balance[4]+" MATIC )",{type:"error"})
          return;
        }
      }else{
        if(price > balance[5]){
          toast("Your TST balance is lower(current - "+balance[5]+" TST )",{type:"error"})
          return;
        }
      }
      // newly added 12/23
  ...
  const LoadNFTs  = async(asset_id)=>{
   ...
   const getBalance = async()=>{
    if(user_address){
      ...
      // newly added 1/3/2023
      const balance_m = await getMATICBalance(eth_addr)
      price_list[4] = balance_m

      const balance_t = await getUSDTBalanceForMatic(eth_addr)
      price_list[5] = balance_t
 ...
 const onChangeCurrency = (val) =>{
  ...
  else if(val == 'usdc' || val == 'usdt' || val == 'tst')
  {
